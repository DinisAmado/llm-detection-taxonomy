...
# Adicionado o import do script de analytics
...
# Entity-type colour palette for nodes (Corrigido para Title Case)
ENTITY_TYPE_COLOR = {
    "Person":       "#4e79a7",
    "Institution":  "#f28e2b",
    "Location":     "#59a14f",
...
SEVERITY_ORDER = list(CATEGORY_COLOR.keys())   # most-severe first
...
def fetch_relations(
    ...
    """Return relation rows joined with their parent result metadata."""
    conditions, params = [], []
...
def fetch_entity_types(conn: sqlite3.Connection) -> dict[str, str]:
    """Return {entity_id: entity_type} for all entities."""
    rows = conn.execute("SELECT entity_id, entity_type FROM entities").fetchall()
    # If multiple types for same id, keep first occurrence
...
def build_graph(
    ...
    """Build a directed multigraph from relation rows."""
    G = nx.MultiDiGraph()
    ...
    # Optionally keep only the top-N most connected entities
...
def export_html(G: nx.MultiDiGraph, out_path: Path):
    """Render an interactive vis.js graph via pyvis."""
    net = Network(
        height="800px",
        width="100%",
        directed=True,
    ...
    for src, tgt, attrs in G.edges(data=True):
        ...
        net.add_edge(
            src, tgt,
            title=f"<b>{itype}</b><br>Category: {cat}<br>Confidence: {score_str}",
            label=itype[:20] if itype else "",
            color={"color": color, "opacity": 0.75},
            arrows="to",
            smooth={"type": "curvedCW", "roundness": 0.2},
        )

    # Inject a legend panel as custom HTML
    legend_items = "".join(
        f'<span style="display:inline-block;width:12px;height:12px;'
        f'background:{c};border-radius:50%;margin-right:4px"></span>{cat}&nbsp;&nbsp;'
...
<div style="position:fixed;bottom:12px;left:12px;background:#0d0d1a;
  ...
  <b style="color:#fff">Edge severity</b><br><br>{legend_items}
</div>
</body>""",
    )
...
# ── Static PNG (networkx + matplotlib) ────────────────────────────────────────

def export_png(G: nx.MultiDiGraph, out_path: Path):
    """Render a static knowledge-graph image."""
    ...
    # Layout – use spring for small graphs, kamada_kawai for larger ones
    ...
    # Entity type (node colours)
    ent_patches = [
        mpatches.Patch(color=c, label=etype)
        for etype, c in ENTITY_TYPE_COLOR.items()
    ]
    leg2 = ax.legend(
        handles=ent_patches,
        title="Entity Type",
...
# ── Per-category subgraphs ────────────────────────────────────────────────────

def export_subgraphs(G: nx.MultiDiGraph, out_dir: Path):
    """Export one PNG per taxonomy category that has enough nodes."""
    ...
    for cat in categories_present:
        # Subgraph: keep only edges of this category
        sub_edges = [
        ...
        if not sub_edges:
            continue
        SG = G.edge_subgraph(sub_edges).copy()
        if SG.number_of_nodes() < 2:
            continue

        sub_png = out_dir / f"subgraph_{cat.lower()}.png"
        # Temporarily override title for subgraph
        _export_subgraph_png(SG, sub_png, title=f"SNA — {cat}")


def _export_subgraph_png(G: nx.MultiDiGraph, out_path: Path, title: str):
    ...
    degrees    = dict(G.degree())
    max_deg    = max(degrees.values(), default=1)
    node_sizes = [200 + 1500 * (degrees[n] / max_deg) for n in G.nodes()]
    node_colors = [G.nodes[n].get("color", "#aaa") for n in G.nodes()]
    edge_colors = [d.get("color", "#666") for _, _, d in G.edges(data=True)]

    nx.draw_networkx_nodes(G, pos, ax=ax, node_size=node_sizes,
                           node_color=node_colors, alpha=0.9,
                           linewidths=0.4, edgecolors="#ffffff22")
    nx.draw_networkx_edges(G, pos, ax=ax, edge_color=edge_colors,
                           alpha=0.6, arrows=True, arrowsize=10,
                           width=0.8, connectionstyle="arc3,rad=0.1")
    nx.draw_networkx_labels(G, pos, ax=ax, font_size=7,
                            font_color="#eeeeee", font_family="monospace")

    ax.set_title(title, color="#ffffff", fontsize=16, pad=12, fontweight="bold")
    plt.tight_layout()
    out_path.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(out_path, dpi=130, bbox_inches="tight", facecolor=fig.get_facecolor())
    plt.close(fig)
    print(f"[✓] Subgraph PNG       → {out_path}")
...
def print_stats(G: nx.MultiDiGraph):
    ...
    if G.number_of_nodes():
        ...
        print(f"  Top-5 nodes : {[n for n, _ in degrees[:5]]}")

    cat_counts = Counter(d["category"] for _, _, d in G.edges(data=True))
    print("\n  Edge distribution by severity:")
    for cat in SEVERITY_ORDER:
        if cat in cat_counts:
            bar = "█" * min(40, cat_counts[cat])
            print(f"    {cat:<14} {cat_counts[cat]:>5}  {bar}")
    print()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build knowledge graphs from sna.db",
    ...
    parser.add_argument("--lang",         nargs="*",
                        choices=["EN", "ES", "PT", "FR"],
                        help="Filter by language(s). E.g. --lang PT EN")
    parser.add_argument("--category",     nargs="*",
                        choices=list(CATEGORY_COLOR),
                        help="Filter by taxonomy category(ies). E.g. --category HATE THREAT")
    parser.add_argument("--limit",        type=int, default=1000,
                        help="Max relations to load (default: 1000)")
    parser.add_argument("--top-entities", type=int, default=None,
                        help="Keep only the N most-connected entities")
    ...
    parser.add_argument("--subgraphs",    action="store_true",
                        help="Also export one PNG per taxonomy category")
...
def main():
    ...
    print(f"[•] Fetching relations (limit={args.limit}, lang={args.lang}, category={args.category})")
    relations    = fetch_relations(conn, langs=args.lang, categories=args.category, limit=args.limit)
    entity_types = fetch_entity_types(conn)
    conn.close()

    print(f"Loaded {len(relations)} relations / {len(entity_types)} entity-type mappings")

    if not relations:
        print("No relations found for the given filters. Exiting.")
        return

    print("Building graph …")
    G = build_graph(relations, entity_types, top_entities=args.top_entities)
    
    # ADICIONADO: Calcula as métricas de influência e exporta para o Streamlit
    ...
    if args.subgraphs:
        export_subgraphs(G, out_dir / "subgraphs")
